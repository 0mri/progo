...
module.exports = function (app, passport) {
  ...
  app.post("/login", UserController.isNotLoggedIn, function (req, res, next) {
    passport.authenticate("local-login", function (err, user, info) {
      if (req.body.csi == 'true') 
        req.session.cookie.maxAge = 3.6e+6 * 24 * 7; //stay sign in for 7 days
      req.logIn(user, function (error) {
        res.json({user, info, err});
      });
    })(req, res, next);
  });
  // SIGNUP =================================
  ...
  app.post("/profile", UserController.isLoggedIn, function (req, res, next) {
    ...
    if (formData.firstName) 
      updtUser.firstName = formData.firstName;
    if (formData.lastName) 
      updtUser.lastName = formData.lastName;
    if (formData.email) {
    ...
    if (formData.password) 
      updtUser.password = User().generateHash(formData.password);
  ...
  app.get("/api/chat/:userid", UserController.isLoggedIn, function (req, res, next) {
    ConversationController.getConversation(req, res, function (err, conversation) {
      ...
      if (conversation) 
        return res.status(200).json(conversation.conversation._id);
  ...
  app.get("/conversations/:userid", UserController.isLoggedIn, function (req, res, next) {
    if (req.params.userid == req.user._id) 
      return res.redirect("/YouCantTalkToYourself");

...
module.exports = {
  getConversations: function (req, res, next) {
    ...
    Conversation.find({participants: req.user._id}).select('_id').exec(function (err, conversations) {
      ...
      conversations.forEach(function (conversation) {
        Message.find({'conversationId': conversation._id}).sort('-createdAt').limit(1).populate({path: "author", select: "firstName lastName"}).exec(function (err, message) {
          if (err) {
            res.send({error: err});
            return next(err);
          }
  ...
  createAndSendMessage: function (message, done) {
    var userId = message.from,
      ...
      msg = message.message;
    if (!recipient) 
      return done({error: 'You have to send it to someone'})
    if (!msg) 
      return done({error: 'You cant send empty message, whats the point?'})
    createConversation([
      userId, recipient
